 ...
 */
const Omniout = ({omniScriptType, subType, language, params, vfpage, style, onMessage}) => {
  const el = useRef();    
  const {sf} = useSf();
  ...
  const uri = sf.omniOutUri({omniScriptType, subType, language, params}, vfpage);
  const source = {uri};
