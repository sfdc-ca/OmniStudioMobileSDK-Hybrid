...
const Lwc = forwardRef(
  (
    ...
    const viewOnMessage = async (event) => {
      ...
      if (onLwcLoad && eventData.name === 'loaded') {
        onLwcLoad();
      }
       ...
       */
      if ((eventData.name === 'omnipostmessage' && eventData.callId === 'omnipostmessage') ||
        ...
        if (onOmniscriptCancel && callbackParams.params.OmniEleName === 'CANCEL') {
          onOmniscriptCancel(callbackParams);
        }

        if (onOmniscriptApiResponse && callbackParams.params.apiResponse) {
          onOmniscriptApiResponse(callbackParams.params.apiResponse, callbackParams);
        }
      }
